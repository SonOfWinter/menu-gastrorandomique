...
export const generateDish = (
  ...
  const ingredients: Ingredient[] = typeAliments && Array.isArray(typeAliments)
    ...
    : [];
  const main = generateMain(data, platPrincipal, ingredients, mainType, inconsistentLevel, rng, selectedIngredients);
  const second = generateSecond(data, platPrincipal, ingredients, mainType, inconsistentLevel, rng, selectedIngredients);
